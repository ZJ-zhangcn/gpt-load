export default {
  ...
  proxyPool: {
    ...
    title: "代理节点池",
    description:
      "管理可复用的出站节点，在密钥分组中进行均衡绑定。当前页面先维护节点清单，健康探测能力将独立接入。",
    overviewTitle: "节点概览",
    totalNodes: "全部节点",
    bindableNodes: "可用于绑定",
    protocolTypes: "协议类型",
    ...
    healthTitle: "检测状态",
    healthLink: "能力说明",
    healthUnavailable: "自动健康检测尚未接入",
    healthHint: "节点导入后可直接用于密钥组均衡绑定；检测接口接入后将在这里展示队列与延迟。",
    healthBoundHint: "删除节点会同步清空关联 Key 的专属代理并回退到分组或环境代理。",
    empty: "还没有代理节点",
